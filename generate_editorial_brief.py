...
def build_brief(player: dict, payload: dict) -> str:
    ...
    L += [
        ...
        f"Usa i dati del brief qui sopra per scrivere 4 note editoriali in italiano",
        f"per il report di **{subject_name}**.",
        "",
        "**Regole:**",
        "- Tono: interpretativo, editoriale. Guida il lettore, non elencare dati.",
        "- Lunghezza: 2–3 frasi compatte per nota.",
        ...
        "",
        "**Una nota per ciascuna sezione:**",
        "",
        f"1. **note_confronto** — Come si colloca {subject_name} rispetto ai",
        f"   {role_plural} dell'{target_team}? Complementarietà o ridondanza?",
        "",
        f"2. **note_heatmap** — Cosa rivela l'impronta spaziale sullo stile",
        f"   e sul raggio d'azione effettivo del giocatore?",
        "",
        f"3. **note_context** — Cosa dice il confronto con i compagni al {source_club}",
        f"   su chi è davvero {subject_name} fuori dal contesto narrativo?",
        "",
        f"4. **note_similarity** — Chi assomiglia di più a {subject_name} tra i",
...
def main():
    ...
    print(f"  4. Copia i 4 campi JSON in assets/data/player_index.json")
