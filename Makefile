...
.PHONY: find-player find-peers find-squad-role-peers find-team find-target-role-peers role-report role-report-note role-report-validate
...
role-report:
	$(PYTHON) scripts/orchestrate_role_report.py --mode export --role $(ROLE) --player-id $(PLAYER_ID) --squad-role-peer-ids "$(PEERS)" $(if $(TARGET_TEAM),--target-team "$(TARGET_TEAM)") $(if $(TARGET_TEAM_ID),--target-team-id $(TARGET_TEAM_ID)) $(if $(TARGET_ROLE_PEERS),--target-role-peer-ids "$(TARGET_ROLE_PEERS)") $(if $(NOTE),--editorial-note "$(NOTE)") $(if $(NOTES_FILE),--editorial-notes-file "$(NOTES_FILE)") --season $(SEASON)

role-report-note:
	$(PYTHON) scripts/orchestrate_role_report.py --mode note-only --role $(ROLE) --player-id $(PLAYER_ID) --squad-role-peer-ids "$(PEERS)" $(if $(TARGET_TEAM),--target-team "$(TARGET_TEAM)") $(if $(TARGET_TEAM_ID),--target-team-id $(TARGET_TEAM_ID)) $(if $(TARGET_ROLE_PEERS),--target-role-peer-ids "$(TARGET_ROLE_PEERS)") $(if $(NOTE),--editorial-note "$(NOTE)") $(if $(NOTES_FILE),--editorial-notes-file "$(NOTES_FILE)") --season $(SEASON)
